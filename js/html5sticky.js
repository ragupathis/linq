...
function save_note(username,note){

$.ajax({
        type: "POST",
		url: 'model/save_sticky.php',			
		data: {'username':username,'notes':note}
...
html5sticky.addUtilityButtons = function(el){
  ...
  /* $('.icons-footer').append('<img class="left shrink ficon" src="img/shrink.png" alt="" title="shrink/expand">');
   $('.icons-footer').append('<img class="left alarm ficon" src="img/alarm.png" alt="" title="set alarm for this sticky note">');
   $('.icons-footer').append('<img class="left lock ficon" src="img/lock.png" alt="" title="lock this sticky note">');
   $('.icons-footer').append('<img class="left settings ficon" src="img/settings.png" alt="" title="settings for note">');
   $('.icons-footer').append('<img class="left email ficon" src="img/email.png" alt="" title="email this sticky note">');
   $('.icons-footer').append('<img class="left tweet ficon" src="img/tweet.png" alt="" title="tweet this sticky note">');
   $('.icons-footer').append('<img class="left share ficon" src="img/share.png" alt="" title="share this sticky note">');
*/
...
html5sticky.editNote = function($clone, el){
   ...
   $clone.find('h2').replaceWith('<input type="text" class="hedit" readonly="true"  />');

   $('.hedit').addClass('inset').val(html5sticky.stripTags(htext, allowed_tags)).width(250);


   // put in Close button
   $('<a href="#" class="close_stickynote"><img src="img/deletebig.png" alt="" title="close note"></a>')
      .css({
         position:'absolute',
         top:7,
         right:5
   ...
   // put in Save button
   $('<a href="#" class="save_stickynote"><img src="img/savebig.png" alt="" title="save note"></a>')
      .css({
         position:'absolute',
         top:7,
         right:60
...
html5sticky.addPin = function(el){
    var close = $('<div><a href="#" class="delete_stickynote"><img src="img/delete.png" alt="" title="delete note"></a></div>');
    var tag = $('<div align="center"><img src="img/pin.png" alt=""></div>');
...
html5sticky.enlargeNote = function(el){
      ...
      if (Modernizr.localstorage){
         ...
         var timeImg = '<img class="left" align="absmiddle" src="img/time.png">';

         dateStr = dateTime.split('|')[0];
         dateAgo = prettyDate(dateTime.split('|')[1]);

      //   dateStr = (dateStr.length > 0) ? 'Time : ' + dateStr : ' ';
       //  dateAgo = (dateAgo.length > 0) ? ' ('  ')' : '';	// ' dateAgo ' place it b/w +
...
html5sticky.Note =function(el){
  ...
  $('.note_common').remove();
    ...
    //  var htext = html5sticky.stripTags($(el).closest('.bignote').find('.hedit')[0].value, allowed_tags);
     ...
     // ptext = ptext.replace(/\r?\n/g, '<br />');
  //  alert('s');  
      localStorage.setItem(identifier + '|text', "hg" + '|' + "h");

      $('[id^=idf_' + identifier + ']').closest('.note_common').find('h2').text("hi");
      $('[id^=idf_' + identifier + ']').closest('.note_common').find('p').html("hello");
    //  alert('Sticky Note Save :)');
    ...
    var stickynote = $('<div class="note_common ' + bgcolor + ' ' + rotationClass + '" title="click here to edit"  />').appendTo($('#main'));
    // add tape to stickynote
    html5sticky.addPin(stickynote);
